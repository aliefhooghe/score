...
#include <Scenario/Commands/Scenario/Creations/CreateEventAfterEventOnSynchronization.hpp>
...
class CreateEventAfterEventOnSynchronizationTest : public QObject
{
  Q_OBJECT
private slots:
  void CreateTest()
  {
    ...
    auto endTn_id = eventCmd.createdSynchronization();

    data.eventClickedId = scenar->startEvent()->id();
    data.relativeY = 0.4;
    data.endSynchronizationId = endTn_id;

    CreateEventAfterEventOnSynchronization cmd(
        {
            {"ScenarioModel", {0}},
        },
        scenar->startEvent()->id(), eventCmd.createdSynchronization(),
    ...
    QCOMPARE((int)scenar->synchronizations().size(), 2);
    try
    {
...
QTEST_MAIN(CreateEventAfterEventOnSynchronizationTest)
#include "CreateEventAfterEventOnSynchronizationTest.moc"
