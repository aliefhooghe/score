...
RemoveSelection::RemoveSelection(
    ...
    : m_path{scenar}
{
  // Serialize all the events and intervals and synchronizations and states and
  // comments
  ...
  // Wa add Synchronization to the list if all its node are selected for deletion
  ...
  // Then add Synchronization to selection if all its Events are gonna be deleted
  cp = sel;
  for (const auto& obj : cp) // Make a copy
  {
    if (auto event = dynamic_cast<const EventModel*>(obj.data()))
    {
      auto& ts = scenar.synchronizations.at(event->synchronization());
      bool add_event=true;
  ...
  for (const auto& obj : purged)
  {
    ...
    if (auto ts = dynamic_cast<const SynchronizationModel*>(obj))
    {
      if (ts->id() != Id<SynchronizationModel>{0})
      {
        QByteArray arr;
        DataStream::Serializer s2{&arr};
        s2.readFrom(*ts);
        m_removedSynchronizations.push_back({ts->id(), arr});
      }
    }
...
void RemoveSelection::undo(const score::DocumentContext& ctx) const
{
  ...
  QList<SynchronizationModel*> synchronizations;
  std::transform(
      m_removedSynchronizations.begin(),
      m_removedSynchronizations.end(),
      std::back_inserter(synchronizations),
      [&](const auto& tndata) {
        DataStream::Deserializer s{tndata.second};
        return new SynchronizationModel{s, &scenar};
      });
  ...
  // Recreate all the removed synchronizations or only restore their trigger
  for (auto& synchronization : synchronizations)
  {
    auto ts = scenar.findSynchronization(synchronization->id());
    ...
    if (ts){
      ...
      auto events_in_synchronization = synchronization->events();
      for (auto& event : events_in_synchronization)
      {
        synchronization->removeEvent(event);
      }
      scenar.synchronizations.add(synchronization);
    }
  }

  // Recreate first all the events / maybe removed synchronizations
  for (auto& event : events)
  {
    ...
    if (ev)
    {
      ...
      // might add a synchronization.
      auto synchronizations_in_scenar = shallow_copy(scenar.synchronizations.map());
      auto scenar_synchronization_it = std::find(
                                  synchronizations_in_scenar.begin(),
                                  synchronizations_in_scenar.end(),
                                  event->synchronization());
      if (scenar_synchronization_it != synchronizations_in_scenar.end())
      {
        ...
        // problems ?
        (*scenar_synchronization_it)->addEvent(event->id());
      }
    }
  }
  ...
  for (auto& tn : scenar.synchronizations)
  {
...
void RemoveSelection::redo(const score::DocumentContext& ctx) const
{
  ...
  for (const auto& tsid : m_removedSynchronizations)
  {
    auto ts = scenar.findSynchronization(tsid.first);
    if (ts->active())
    {
      ts->setActive(false);
    }
...
void RemoveSelection::serializeImpl(DataStreamInput& s) const
{
  s << m_path << m_removedEvents
    << m_removedSynchronizations << m_removedIntervals << m_removedStates
    << m_removedComments;
}
